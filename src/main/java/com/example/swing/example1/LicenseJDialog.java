...
public class LicenseJDialog extends javax.swing.JDialog {
    ...
    public void checkLicense(String server) {
        SwingWorker worker = new SwingWorker<>() {
            @Override
            protected Void doInBackground() throws Exception {
                // Save Floating License Server address to some config file to use on the next startup.
                File licenseServerSave = new File(System.getProperty("user.home") + File.separator + "fls-address.conf");
                FileUtils.writeStringToFile(licenseServerSave, server, Charset.defaultCharset());
                ...
                License.getInstance().getBuilder()
                        ...
                        .build();
                /**
                 * validate on the given floating license server.
                 */
    ...
    private void changeLicenseKeyjButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_changeLicenseKeyjButtonActionPerformed
        // Ask for the floating license server address.
        // e.g. http://192.168.1.100:16090/fls
        String server = JOptionPane.showInputDialog("Floating License Server Address");
        if (server != null) {
