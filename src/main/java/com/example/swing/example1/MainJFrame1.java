...
/**
 * Floating Liceense validation example. Install a floating license server to
 * test. https://www.license4j.com/products/floating-license-server/
 *
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        SwingWorker worker = new SwingWorker<>() {
            ...
            protected Void doInBackground() throws Exception {
                // At first run, ask for floating license serve to the user, and save it in a config file.
                // On each run, read the config file and get the floating license server address for validatin.
                File licenseServerSave = new File(System.getProperty("user.home") + File.separator + "fls-address.conf");
                
                if (licenseServerSave.exists()) {
                    String server = FileUtils.readFileToString(licenseServerSave, Charset.defaultCharset());
